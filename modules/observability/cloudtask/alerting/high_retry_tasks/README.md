...
- **`auto_close` is validated:** it must match Cloud Monitoring's `Duration` format — digits followed by `s` (e.g. `"86400s"`). `terraform plan` rejects values like `"1d"` instead of failing at `apply` against the GCP API.
